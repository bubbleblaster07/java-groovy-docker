node('master')   {
      def dockerImageName= 'sohail/javadedockerapp_$JOB_NAME:$BUILD_NUMBER'
      stage('SCM Checkout'){
         git 'https://github.com/bubbleblaster07/java-groovy-docker.git'          
      }
      stage('Build'){
     ...
     stage('Build Docker Image'){         
      ...
      stage('Publish Docker Image'){
         withCredentials([string(credentialsId: 'dockerpwd', variable: 'dockerPWD')]) {
              sh "docker login -u sohail -p ${dockerPWD}"
         }
